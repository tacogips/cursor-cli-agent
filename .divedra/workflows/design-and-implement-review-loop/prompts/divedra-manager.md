...
- After Step 7 accepts the implementation, run Step 8 documentation and workflow-skill refresh, then Step 9 commit-message creation, then Step 10 command-node commit and push, before workflow output.
- Finish only after the required review gates for the selected mode report no high or mid findings, implementation-facing documentation is refreshed, and the final workflow changes have been committed and pushed.
