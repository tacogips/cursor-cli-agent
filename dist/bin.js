...
import { mkdir, readFile as readFile8, rename as rename4, writeFile as writeFile6 } from "fs/promises";
import { randomUUID as randomUUID8 } from "crypto";
import { join as join6 } from "path";
var TOKENS_FILE = "tokens.json";
function tokenFilePath(configDir) {
  return join6(configDir, TOKENS_FILE);
}
...
import { createHash as createHash2, randomUUID as randomUUID10 } from "crypto";
...
async function executeSessionReplayFork(request, headlessBase, deps) {
  ...
  const replayForkId = randomUUID11();
...
import { basename as basename2, dirname as dirname8, join as join7 } from "path";
function skillRoots(opts) {
  const cursor = getCursorHome();
  const out = [
    { root: join7(cursor, "skills-cursor"), scope: "builtin" },
    { root: join7(cursor, "skills"), scope: "user" }
  ];
  if (opts.projectRoot !== undefined && opts.projectRoot.length > 0) {
    out.push({
      root: join7(opts.projectRoot, ".cursor/skills"),
      scope: "project"
    });
  }
  return out;
...
async function listSkillRecords(opts = {}) {
  ...
  for (const { root, scope } of skillRoots(opts)) {
    ...
    for (const path of files) {
      let text;
      try {
        text = await readFile9(path, "utf8");
      } catch {
        continue;
      }
...
async function save5(path, data) {
  ...
  const tmpPath = `${path}.tmp.${randomUUID12().slice(0, 8)}`;
